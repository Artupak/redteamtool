...
class PhishingModule(BaseModule):
    def __init__(self, name: str, description: str):
        super().__init__(name, description or "Simulates phishing attacks for security awareness testing")
        self.author = "ARTUPAK"
    ...
    def run(self, target: str, **kwargs) -> bool:
        ...
        try:
            ...
            for target_data in targets: # Renamed target to avoid conflict with outer scope target
                try:
                    if self._send_phishing_email(target_data, template):
                        success_count += 1
                    else:
                        fail_count += 1
                except Exception as e:
                    self.logger.error(f"Error sending email to {target_data['email']}: {str(e)}")
                    fail_count += 1
    ...
    def _send_phishing_email(self, target_data: Dict[str, str], template: jinja2.Template) -> bool: # Renamed target to avoid conflict
        """
        ...
        try:
            ...
            self.logger.info(f"Successfully sent phishing email to {target_data['email']}")
            return True

        except Exception as e:
            self.logger.error(f"Failed to send phishing email to {target_data['email']}: {str(e)}")
            return False
